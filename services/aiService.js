...
export default async function generateSQL(question, schema, overriddenProvider = null) {
  ...
  const prompt = `
You are a MySQL expert and Data Analyst. Use the provided database schema to generate a SQL query based on the user's question. 
Also, determine if the resulting data should be visualized.
...
INSTRUCTIONS:
1. Generate a valid MySQL query that answers the question.
2. Determine if the data is suitable for a chart. If so, choose a chart type: "bar", "line", or "pie".
3. Identify which columns from your SQL query should be used for the X-axis and Y-axis.
...
- Use "bar" for comparing categories (e.g., counts by city, sales by product).
- Use "line" for trends over time (e.g., users joined by month, revenue by day).
- Use "pie" for part-to-whole relationships (e.g., percentage of users by role).
- Use "none" for simple lists, structure descriptions, or single values where a chart doesn't make sense.
- xAxis should be the categorical or time-based column.
- yAxis should be the numerical/metric column.

CRITICAL:
- Return ONLY the JSON. No other text.
- Use backticks (\`) for table and column names in the SQL.
- If the question is about table structure (DESCRIBE), set chartType to "none".
`;

  let response;
  try {
    // OpenAI-compatible format (via ai_server.py)
