...
function App() {
  ...
  const { fetchSessions } = useOutletContext();
  ...
  // Fetch specific chat session when chatId changes
  useEffect(() => {
    if (chatId) {
  ...
  const fetchSessionHistory = async (id) => {
    try {
      const res = await fetch(`http://localhost:5000/api/chats/session/${id}`);
  ...
  const askDatabase = async () => {
    ...
    try {
      ...
      const res = await fetch("http://localhost:5000/api/query", {
        ...
        body: JSON.stringify({ question, userId, aiProvider, chatId }), 
      });
      ...
      // If it was a new chat, redirect to the new chatId and refresh sessions
      if (!chatId && data.chatId) {
        navigate(`/dashboard/chat/${data.chatId}`);
        fetchSessions(userId);
      } else if (chatId) {
        // Just refresh sessions to update the "updatedAt" and title if changed
        fetchSessions(userId);
      }
    } catch (error) {
      setMessages((prev) => [
