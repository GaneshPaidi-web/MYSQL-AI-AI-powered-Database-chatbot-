  ...
  async getSchema() {
    try {
      ...
      if (columns.length === 0) {
        ...
        const [currentDbRows] = await this.pool.query("SELECT DATABASE() as db");
        const currentDbName = currentDbRows[0]?.db;

        if (!currentDbName) {
          return "No database selected. Global queries like SHOW DATABASES or CREATE DATABASE can be run.";
        }
        ...
        try {
          ...
          // Ignore error if SHOW TABLES fails for some other reason
        }

        return `Database '${currentDbName}' is empty or no tables found.`;
      }
